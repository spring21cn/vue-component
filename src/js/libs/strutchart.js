!(function(name, context, definition) {
	...
	var StrutChart = function(options) {
	...
	StrutChart.prototype.getName = function() {
		return this._name;
	}
	StrutChart.prototype._closest = function(el, fn) {
		return el && ((fn(el) && el !== this.chart) ? el : this._closest(el.parentNode, fn));
	}
	StrutChart.prototype._siblings = function(el, expr) {
	...
	StrutChart.prototype._prevAll = function(el, expr) {
		var sibs = []
	...
	StrutChart.prototype._nextAll = function(el, expr) {
		var sibs = [];
	...
	StrutChart.prototype._isVisible = function(el) {
		return el.offsetParent !== null;
	}
	StrutChart.prototype._addClass = function(elements, classNames) {
	...
	StrutChart.prototype._removeClass = function(elements, classNames) {
		elements.forEach(function(el) {
			if (classNames.indexOf(' ') > 0) {
				classNames.split(' ').forEach(function(className) {
	...
	StrutChart.prototype._css = function(elements, prop, val) {
		elements.forEach(function(el) {
			el.style[prop] = val;
		});
	}
	StrutChart.prototype._removeAttr = function(elements, attr) {
		elements.forEach(function(el) {
			el.removeAttribute(attr);
		});
	}
	StrutChart.prototype._one = function(el, type, listener, self) {
	...
	StrutChart.prototype._getDescElements = function(ancestors, selector) {
		var results = [];
	...
	StrutChart.prototype._getJSON = function(url) {
	...
	StrutChart.prototype._buildJsonDS = function(li) {
		var subObj = {
	...
	StrutChart.prototype._attachRel = function(data, flags) {
		data.relationship = flags + (data.children && data.children.length > 0 ? 1 : 0);
		if (data.children) {
			for (var i = 0, j = data.children.length; i < j; i++) {
				var item = data.children[i];
				this._attachRel(item, '1' + (j > 1 ? 1 : 0));
			}
		}
		return data;
	}
	StrutChart.prototype._repaint = function(node) {
		if (node) {
			node.style.offsetWidth = node.offsetWidth;
		}
	}
	StrutChart.prototype._isInAction = function(node) {
		return node.querySelector('.edge').className.indexOf('fa-') > -1;
	}
	StrutChart.prototype._getNodeState = function(node, relation) {
		var criteria, state = {
			'exist': false,
			'visible': false
		};
	...
	StrutChart.prototype.getRelatedNodes = function(node, relation) {
		if (relation === 'parent') {
	...
	StrutChart.prototype._switchHorizontalArrow = function(node) {
	...
	StrutChart.prototype._hoverNode = function(event) {
	...
	StrutChart.prototype._clickNode = function(event) {
	...
	StrutChart.prototype._buildParentNode = function(currentRoot, nodeData, callback) {
		var that = this
	...
	StrutChart.prototype._switchVerticalArrow = function(arrow) {
		arrow.classList.toggle('fa-chevron-up');
		arrow.classList.toggle('fa-chevron-down');
	}
	StrutChart.prototype.showParent = function(node) {
	...
	StrutChart.prototype.showSiblings = function(node, direction) {
		var siblings = []
		  , temp = this._closest(node, function(el) {
			return el.nodeName === 'TABLE'
		}).parentNode;
	...
	StrutChart.prototype.hideSiblings = function(node, direction) {
	...
	StrutChart.prototype.hideParent = function(node) {
	...
	StrutChart.prototype.addParent = function(currentRoot, data) {
		var that = this;
	...
	StrutChart.prototype._startLoading = function(arrow, node) {
		var opts = this.options
		  , chart = this.chart;
	...
	StrutChart.prototype._endLoading = function(arrow, node) {
		var opts = this.options;
	...
	StrutChart.prototype._clickTopEdge = function(event) {
	...
	StrutChart.prototype.hideChildren = function(node) {
		var that = this
	...
	StrutChart.prototype.showChildren = function(node) {
		var that = this
		  , temp = this._nextAll(node.parentNode.parentNode)
		  , descendants = [];
		this._removeClass(temp, 'hidden');
	...
	StrutChart.prototype._buildChildNode = function(appendTo, nodeData, callback) {
	...
	StrutChart.prototype.addChildren = function(node, data) {
		var that = this
		  , opts = this.options
		  , count = 0;
	...
	StrutChart.prototype._clickBottomEdge = function(event) {
		event.stopPropagation();
		var that = this
		  , opts = this.options
	...
	StrutChart.prototype._complementLine = function(oneSibling, siblingCount, existingSibligCount) {
	...
	StrutChart.prototype._buildSiblingNode = function(nodeChart, nodeData, callback) {
		var that = this
	...
	StrutChart.prototype.addSiblings = function(node, data) {
		var that = this;
	...
	StrutChart.prototype.removeNodes = function(node) {
	...
	StrutChart.prototype._clickHorizontalEdge = function(event) {
		event.stopPropagation();
		var that = this
		  , opts = this.options
		  , hEdge = event.target
	...
	StrutChart.prototype._clickToggleButton = function(event) {
		var that = this
	...
	StrutChart.prototype._dispatchClickEvent = function(event) {
	...
	StrutChart.prototype._onDragStart = function(event) {
		...
		var dragHier = Arrayfrom(this._closest(dragged, function(el) {
			return el.nodeName === 'TABLE';
	...
	StrutChart.prototype._onDrop = function(event) {
		...
		if (!!window.ActiveXObject || 'ActiveXObject'in window) {
			...
			customE.initEvent('nodedropped.strutchart', {
				'detail': {
					'draggedNode': dragged,
	...
	StrutChart.prototype._createNode = function(nodeData, level) {
		var that = this
		  , opts = this.options;
		return new Promise(function(resolve, reject) {
			if (nodeData.children) {
				for (var i = 0, j = nodeData.children.length; i < j; i++) {
	...
	StrutChart.prototype.buildHierarchy = function(appendTo, nodeData, level, callback) {
		...
		if (childNodes) {
			...
			var isHidden, isVerticalLayer = opts.verticalDepth && (level + 2) >= opts.verticalDepth, inEdit = that.chart.dataset.inEdit;
			if (inEdit) {
	...
	StrutChart.prototype._clickChart = function(event) {
	...
	StrutChart.prototype._clickExportButton = function() {
		var opts = this.options
	...
	StrutChart.prototype._loopChart = function(chart) {
		var subObj = {
	...
	StrutChart.prototype._onPanning = function(event) {
		var chart = event.currentTarget;
	...
	StrutChart.prototype._onPanEnd = function(event) {
	...
	StrutChart.prototype._setChartScale = function(chart, newScale) {
		var lastTf = window.getComputedStyle(chart).transform;
		if (lastTf === 'none') {
	...
	StrutChart.prototype._onWheeling = function(event) {
		var newScale = event.deltaY > 0 ? 0.8 : 1.2;
		this._setChartScale(this.chart, newScale);
		event.preventDefault();
	}
	StrutChart.prototype._getPinchDist = function(event) {
	...
	StrutChart.prototype._onTouchStart = function(event) {
		var chart = this.chart;
	...
	StrutChart.prototype._onTouchMove = function(event) {
		var chart = this.chart;
	...
	StrutChart.prototype._onTouchEnd = function(event) {
		var chart = this.chart;
		if (chart.dataset.pinching) {
